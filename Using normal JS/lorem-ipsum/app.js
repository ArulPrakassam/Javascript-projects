...
const text = [
  ...
  `Zombie ipsum reversus ab viral inferno, nam rick grimes malum cerebro. De carne lumbering animata corpora quaeritis. Summus brains si​t, morbo vel maleficia? De apocalypsi gorger omero undead survivor dictum mauris. Hi mindless mortuis soulless creaturas, imo evil stalking monstra adventus resi dentevil vultus comedat cerebella viventium. Qui animated corpse, cricket bat max brucks terribilem incessu zomby. The voodoo sacerdos flesh eater, suscitat mortuos comedere carnem virus. Zonbi tattered for solum oculi eorum defunctis go lum cerebro. Nescio brains an Undead zombies. Sicut malus putrid voodoo horror. Nigh tofth eliv ingdead.`,
...
form.addEventListener("submit", (e) => {
  ...
  if (isNaN(value) || value <= 0) {
    ...
    let tempText = text.slice(0, 9);
    let tempNum = value - 9;
    for (let i = 0; i < tempNum; i++) {
      tempText.push(text[Math.floor(Math.random() * text.length)]);
    }
    display(tempText);
  } else {
